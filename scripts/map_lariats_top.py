...
SCRIPTS_DIR = '/datasets2/lariat_mapping/scripts'
...
@dataclass
class RunData:
    ATTRS = ['fastq_dir', 'scripts_dir', 'log_dir', 'output_dir', 'results_path', 'SB_cpus',
             'SB_GBmem', 'ref_b2index', 'ref_fasta', 'ref_gtf', 'ref_5p_fasta', 'ref_3p_b2index', 'ref_3p_lengths',
    ...
    def validate(self) -> None:
        ...
        for SB_var, SB_val in (('number of cpus', self.SB_cpus),
                               ('memory allocation (in Gb)', self.SB_GBmem)):
            if not SB_val.isdigit() or int(SB_val) < 1:
                raise ValueError(
                    f'The value for the {SB_var} SBATCH setting, {SB_val}, is not a positive integer.')
...
def write_mapping_scripts(sample: dict, args: dict, run_data: RunData, log: Logger) -> None:
    ...
    for read_num in ['one', 'two']:
        ...
        logfile_path = join(run_data.log_dir, 'child_logs',
                            f'{script_name}-$SLURM_JOB_ID.out')
        ...
        with open(script_path, 'w') as script_file:
            ...
            script_file.write('#!/bin/bash -l\n\n'
                              f'#SBATCH --ntasks=1\n'
                              f'#SBATCH --cpus-per-task={run_data.SB_cpus}\n'
                              f'#SBATCH --mem={run_data.SB_GBmem}G\n'
                              f'#SBATCH --job-name={script_name}\n'
                              f'#SBATCH --output={run_data.log_dir}/child_logs/{script_name}-%j.out\n'
                              f'#SBATCH --error={run_data.log_dir}/child_logs/{script_name}-%j.out\n\n')
            ...
            script_file.write('printf "SLURM job id = $SLURM_JOB_ID\\n"\n')
...
def write_sbatch_all(run_data: RunData, log: Logger) -> None:
    '''
    Write a _sbatch_all.sh script in run_data.scripts_dir that submits all larmap*.sh scripts for processing by SLURM
    '''
    sbatch_all_path = join(run_data.scripts_dir, '_sbatch_all.sh')
    with open(sbatch_all_path, 'w') as script_file:
        script_file.write('#!/bin/bash\n\n')
        for sample in run_data.sample_info:
            script_file.write('sbatch ' + join(run_data.scripts_dir,
                              f'larmap_{sample["output_base_name"]}_R1.sh\n'))
            script_file.write('sbatch ' + join(run_data.scripts_dir,
                              f'larmap_{sample["output_base_name"]}_R2.sh\n'))
        # ADDED THIS LINE: WAIT UNTIL EVERY JOB FINISHES?
        script_file.write('wait')

    # Set the child's permissions so everyone group can rwx
    chmod(sbatch_all_path, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)

    log.info(f'Creating top level mapping script at {sbatch_all_path}')
