...
from map_lariats_top import RunData
...
def filter_lariat_reads(merged_lariats: dict, threep_sites:dict, fivep_sites:dict, introns: dict, gene_info: dict, run_data, log) -> dict:
	...
	for sample in merged_lariats:
		...
		map_call = f'bowtie2 --end-to-end --no-unal --threads {run_data.SB_cpus} -k 30 -f -x {run_data.ref_b2index} -U {seq_tmp_fa} -S {seq_tmp_sam}'
