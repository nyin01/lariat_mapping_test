...
def parse_lariat_table(final_info_table: str) -> dict:
    ...
    with open(final_info_table, 'r') as lariat_file:
        ...
        for line in lariat_file:
            ...
            reads[read_id] = [trim_seq, read_seq, chrom, strand, int(
                fivep_site), read_is_reverse, fivep_read_start, fivep_read_end]
            reads[read_id] += [int(threep_site), int(bp_site),
                               read_bp_nt, genomic_bp_nt, genomic_bp_window]
    return reads
...
def get_aligned_reads(run_data, log) -> dict:
    ...
    for sample in run_data.sample_info:
        for read_num in ['one', 'two']:
            ...
            read_count_path = join(
                run_data.output_dir, read_output_name, f'{read_output_name}_total_reads.txt')
            log.info(f'Looking for total reads in {read_count_path}')
            if not isfile(read_count_path):
                raise FileNotFoundError(
                    f'No total reads found for sample "{read_output_name}"')
...
def filter_lariat_reads(merged_lariats: dict, threep_sites: dict, fivep_sites: dict, introns: dict, gene_info: dict, run_data, log) -> dict:
    ...
    for sample in merged_lariats:
        ...
        for rid in merged_lariats[sample]:
            trim_seq, read_seq, chrom, strand, fivep_site, read_is_reverse, fivep_read_start, fivep_read_end, threep_site, bp_site, read_bp_nt, genomic_bp_nt, genomic_bp_window = merged_lariats[
                sample][rid]
            ...
            outside_ss = not fivep_sites[chrom][strand].overlaps(
                bp_site) and not threep_sites[chrom][strand].overlaps(bp_site)
            # The branchpoint is downstream of the 5' splice site
            correct_order = (strand == '+' and fivep_site <
                             bp_site) or (strand == '-' and fivep_site > bp_site)
            ...
            if outside_ss and correct_order and outside_ubiquitin:
                overlap_introns = list(
                    introns[chrom][strand].overlap(bp_site, bp_site+1))
                if len(overlap_introns) > 0:
                    if strand == '+':
                        threep_site = min(
                            overlap_introns, key=lambda s: s.end-bp_site).end
                    else:
                        threep_site = min(
                            overlap_introns, key=lambda s: bp_site-s.begin).begin
                    dist_to_threep = bp_site-threep_site if strand == '+' else threep_site-bp_site
                    trim_reads[rid] = trim_seq
                    filtered_reads[sample][rid] = [gene_data, read_seq, chrom, strand, fivep_site,
        ...
        filtered_reads[sample] = {rid: filtered_reads[sample][rid]
                                  for rid in filtered_reads[sample] if rid not in discard_rids}
        for rid in filtered_reads[sample]:
        ...
        fail_reason_out = ','.join(
            [f'{r} ({fail_reason[r]})' for r in fail_reason])
        log.info(
            f'{sample}: Pre-filter read count = {len(merged_lariats[sample])}, Post-filter read count = {len(filtered_reads[sample])}, Discarded read counts = {fail_reason_out}')
...
if __name__ == '__main__':
    ...
    for sample in run_data.sample_info:
        for read_num in ['one', 'two']:
            read_output_name = f'{sample["output_base_name"]}_{RMAP[read_num]}'
            final_info_table = join(
                run_data.output_dir, read_output_name, f'{read_output_name}_final_info_table.txt')
            lariat_reads[read_output_name] = parse_lariat_table(
                final_info_table)
    ...
    merged_filtered_lariats = filter_lariat_reads(
        merged_lariats, threep_sites, fivep_sites, introns, gene_info, run_data, log)
    ...
    with open(run_data.results_path, 'w') as results_file:
        ...
        for sample in merged_filtered_lariats:
            ...
            sample_info = [
                s for s in run_data.sample_info if s['output_base_name'] == sample][0]
            # Add output categories to the front
            sample_categories = [sample_info[c] for c in output_categories]
            sample_aligned_count = max(
                sample_read_counts[f'{sample}_R1'], sample_read_counts[f'{sample}_R2'])
            for read_info in merged_filtered_lariats[sample].values():
                read_output = sample_categories + \
                    read_info + [sample_aligned_count]
                results_file.write(','.join([str(e)
                                   for e in read_output]) + '\n')
